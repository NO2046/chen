export default {
  "virtualAccount": "Tài khoản ảo",
  appName:'Trợ thủ vay tiền',
  t2:'Có phải bạn gặp phải những vấn đề dưới đây không?',
  c1:'Không tìm được app chính quy',
  c2:'Đăng ký vay luôn bị từ chối',
  c3:'Lãi suất vay quá cao',
  p1:'Nhập số điện thoại di động',
  p11:'Vui lòng điền đúng số điện thoại',
  p2:'Nhập mật khẩu',
  p22:'Vui lòng điền mật khẩu',
  login:'Đăng nhập',
  register:'Đăng ký',
  info:'Đăng ký ngay, giúp bạn giải quyết vấn đề',
  info2:'Chúng tôi hợp tác chiến lược cùng các app lớn cung cấp dịch vụ cá nhân cho bạn',
  p3:'Đăng nhập thành công',
  p4:'Đăng ký thành công',
  p5:'Mật khẩu có ít nhất 6 chữ số',
  p6:'Mục này là bắt buộc',
  p7:'Hai mật khẩu đã nhập không nhất quán',
  p8:'Mật khẩu không đúng',
  p9:'Người dùng không tồn tại, vui lòng đăng ký trước',
  p10:'Xác nhận mật khẩu',

  // page2 

  a1:'Họ tên người dùng',
  a2:'Số chứng minh nhân dân',
  a3:'Địa chỉ nhà ở',
  a4:'Tên ngân hàng',
  a5:'Số tài khoản ngân hàng',
  a6:'Chi nhánh mở thẻ',
  a7:'Thông tin công việc：xxx',
  a8:'Đăng ký',

  // page3
  d1:'Gợi ý sản phẩm',
  d2:'Sản phẩm',
  d3:'Hạn mức vay',
  d4:'Phí phục vụ',
  d5:'Đăng ký vay',
  d6:'triệu',
  d7:'Nhắc nhở thân thiện: Chi tiêu thông minh, đề phòng bội chi',
  d8:'nghìn',



  f1:'Chúc mừng bạn, bạn đã đăng ký vay thành công app Trợ thủ vay tiền',
  f2:'Bạn đã nhận được khoản vay 6 triệu từ app: ',
  f3:'Số tiền thực nhận: ',
  f4:'Chu kỳ vay: ',
  f5:'ngày',
  f6:'Lãi suất vay: ',
  f7:'Số tiền đến hạn thanh toán: ',
  f8:'Phí thủ tục',
  f9:'Thanh toán phí dịch vụ, lập tức giải ngân ngay',
  f10:'Thanh toán',


  

  r1:'Căn cứ theo khoản vay của bạn',
  r2:'số tiền phí dịch vụ bạn cần thanh toán là',
  r3:'Số tài khoản: ',
  r4:'Mở tài khoản nạp tiền',
  r5:'Sau khi thanh toán vui lòng chụp và giữ lại hóa đơn',
  r6:'để thuận tiện cho chúng tôi đối chiếu đồng thời nhanh chóng mở tài khoản cho bạn',
  r7:'Thanh toán'
};


